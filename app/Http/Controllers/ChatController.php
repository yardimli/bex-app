...
use App\Helpers\MyHelper; // Import MyHelper
...
use Illuminate\Support\Facades\Cache; // <-- FIX: Import the Cache facade
...
class ChatController extends Controller
{
    ...
    public function show(Request $request, $chatHeaderId = null)
    {
        ...
        $initialPrompt = null;
        if (!$chatHeader) {
            // Only process initial prompts for new chats
            if ($request->has('prompt')) {
                $initialPrompt = trim($request->input('prompt'));
            } elseif ($request->has('summarize_key')) {
                $summaryKey = $request->input('summarize_key');
                $summaryData = Cache::pull($summaryKey); // Try cache first and delete

                if ($summaryData && is_array($summaryData)) { // Data found in cache (from Workspace summarize button)
                    $prompt = $summaryData['prompt'] ?? 'Summarize this content';
                    $source = $summaryData['source'] ?? 'the provided content';
                    $content = $summaryData['content'] ?? '';
                    $initialPrompt = "{$prompt} from file [[ {$source} ]]\n\n{$content}";

                } elseif (session()->has($summaryKey)) { // Fallback to session (from general Summarize modal)
                    $sessionData = session($summaryKey);
                    if (is_array($sessionData) && isset($sessionData['prompt_text']) && isset($sessionData['full_text'])) {
                        $initialPrompt = $sessionData['prompt_text'] . "\n\n" . $sessionData['full_text'];
                    } else {
                        $initialPrompt = "[Error: Could not load summarization content. Please try again.]";
                        Log::warning("Summarize key '{$summaryKey}' found in session, but data was malformed.");
                    }
                    session()->forget($summaryKey); // Clean up

                } else {
                    // Key not found in cache or session
                    Log::warning("Summarize key '{$summaryKey}' not found in cache or session.");
                    $initialPrompt = "[Error: Content for summarization not found or has expired. Please try again.]";
                }
            }
        }
