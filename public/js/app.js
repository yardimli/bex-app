...
$(document).ready(function () {
    ...
    // MODIFIED: Simplified for DaisyUI drawer component.
    // The drawer is controlled by a checkbox, so JS is only needed for edge cases like closing on overlay click.
    const drawerCheckbox = $('#my-drawer-2');
    const drawerContent = $('.drawer-content');
    drawerContent.on('click', function(e) {
        if (drawerCheckbox.is(':checked') && $(e.target).hasClass('drawer-overlay')) {
            drawerCheckbox.prop('checked', false);
        }
    });
    
    // --- Theme Toggle Logic ---
    // MODIFIED: Manages the `data-theme` attribute for DaisyUI.
    function applyTheme(theme) {
        const newTheme = theme === 'dark' ? 'dark' : 'light';
        htmlElement.attr('data-theme', newTheme);
        themeToggleButton.find('i').removeClass(newTheme === 'dark' ? lightIcon : darkIcon).addClass(newTheme === 'dark' ? darkIcon : lightIcon);
        localStorage.setItem('theme', newTheme);
    }
    
    // Initialize theme on page load based on localStorage or system preference.
    const currentTheme = localStorage.getItem('theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
    applyTheme(currentTheme);
    
    themeToggleButton.on('click', function (e) {
        e.preventDefault();
        const newTheme = htmlElement.attr('data-theme') === 'dark' ? 'light' : 'dark';
        applyTheme(newTheme);
    });
    
    // --- Modal Opening Logic ---
    // MODIFIED: Uses the native `.showModal()` method for DaisyUI's <dialog> elements.
    $('#meetingSummaryButton').on('click', () => recentMeetingsModal.showModal());
    $('#actionItemsButton').on('click', () => actionItemsModal.showModal());
    $('#myNotesButton').on('click', () => myNotesModal.showModal());
    $('#myRecordingsButton').on('click', () => myRecordingsModal.showModal());
    $('#summarizeButton').on('click', () => summarizeContentModal.showModal());
    $('#transcribeButton').on('click', () => transcribeModal.showModal());
    $('#teamWorkspaceButton').on('click', () => teamFilesModal.showModal());
    ...
    function redirectToChatWithSummarizationData(data) {
        ...
        summarizeContentModal.close(); // MODIFIED: DaisyUI close method
    ...
    $('.sidebar .menu').on('click', '.delete-chat-btn', function (e) {
        ...
        if (!chatId) { alert('Error: Could not determine which chat to delete.'); return; }
        ...
        if (confirm(`Are you sure you want to delete the chat "${chatTitle}"? This cannot be undone.`)) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        chatListItem.fadeOut(300, function () { $(this).remove(); });
    ...
    if (attachFileModal.length) {
        ...
        $('#attach-file-btn').on('click', function() {
            ...
            attachFileModal[0].showModal();
        });
        
        attachTeamSelectFilter.on('change', function() { loadTeamFilesForAttachment($(this).val()); });
    ...
    $(document).on('click', '.preview-btn', function() {
        ...
        if (mimeType.startsWith('image/')) {
            ...
            imagePreviewModal.showModal(); // MODIFIED
        } else if (mimeType === 'application/pdf') {
            $('#pdf-preview-content').attr('src', previewUrl);
            pdfPreviewModal.showModal(); // MODIFIED
        }
    });
});
